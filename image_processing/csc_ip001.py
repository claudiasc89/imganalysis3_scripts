...
def projection_4D (img,subfolder):
    'This function performs the desired projection on the image'
    global proj_type
    global maxsd
    global filename
    ...
    # Create a log file to store the information about the z-stack used
    log_file = os.path.join(folder,subfolder, f"{output_name}_{proj_type}_log.txt")
    ...
    if proj_type == 'best':
        ...
        # Check that stack_depth is smaller or equal than image dimensions
        if stack_depth > s_dimensions_map['z']:
            raise ValueError(f"For image {filename} stack depth should be smaller or equal than image z dimensions")
        ...
        # to slice the image
        img_z = img.take(z_range, axis=s_dimensions_indices['z']) # find the axis where z is stored
        
        # Perform the appropiate projection
        
        if proj_type =='mean':
            # use numpy to perform the mean projeaction
            img_proj=np.mean(img_z,axis=s_dimensions_indices['z'], dtype=np.uint16) #I perform mean projection
        elif proj_type =='max':
            # use numpy to perform the mean projeaction
            img_proj=np.max(img_z,axis=s_dimensions_indices['z']) #I perform mean projection
            # Max does not require dtype because it returns the maximum value directly, maintaining the same data type as the input.
        ...
        if "z" in dimensions_map:
            ...
            # For time-lapse images
                ...
                for t in range(dimensions_map['t']):
                    ...
                    sd, maxsd = focused_z(img_tslice) # Get an array with the sd per slice and the position of the maximal sd
                    
                    # Perform the projection and return the projected image
                    img_proj_tp = projection_4D(img_tslice,'projections')
                ...
                # things to do add metadata to stored image 
                # properly write all the functions
                # clone to repository
